...
//! - [`RequireAuth`]: requires a valid HTTP Signature; returns 401 if absent or invalid.
...
/// Axum extractor that requires a valid HTTP Signature.
///
...
fn parse_signature_header(header: &str) -> Result<ParsedSignature, String> {
    let mut key_id = None;
...
fn decode_multibase_key(multibase: &str) -> Result<ed25519_dalek::VerifyingKey, String> {
    let b58 = multibase
        .strip_prefix('z')
...
mod tests {
    ...
    async fn registered_app(_signing_key: &SigningKey, did: &str, multibase: &str) -> (Router, Arc<dyn Storage>) {
        ...
        let app = build_router(Arc::clone(&storage), config);
        (app, storage)
    }
