...
| `derived-from` | This unit's content was derived from the referenced unit. |
...
**Typed references are the primary value of the graph.** A web of pointers that says "these units are related" is marginally useful. A graph where every edge says "A supports B", "C contradicts D", "E derived-from F" allows automated reasoning: path-finding through support chains, detecting contradictions (units that both support and contradict the same claim), finding the assumptions underlying an inference.
...
- `supports` and `contradicts` are the core epistemic operations (agreement and disagreement)
- `derived-from` makes inference chains explicit
...
- The semantic relationship type set is `supports`, `contradicts`, `derived-from`, `questions`, `refines`. The protocol-level type `notifies` is reserved for node infrastructure use.
- Cross-type reference validity (e.g., a `question` unit using `rel: "contradicts"`) is not enforced by the schema; it is left to linting tools and reasoning layers to flag unusual combinations.
